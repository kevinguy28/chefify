...
class Recipe(models.Model):
    ...
    reviewers = models.ManyToManyField(
        User, blank=True, related_name="reviewed_recipes"
    )
    ...
    def update_rating(self):
        ...
        if total_reviews > 0:
            ...
            average = (
                round((sum_ratings / total_reviews) * 2) / 2
...
class RecipeSteps(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.order:
            last_order = RecipeSteps.objects.filter(recipe=self.recipe).aggregate(
                models.Max("order")
            )["order__max"]
...
class UserProfile(models.Model):
    ...
    ownedIngredients = models.ManyToManyField(
        Ingredient, related_name="owned_by", blank=True
    )
    buyIngredients = models.ManyToManyField(
        Ingredient, related_name="to_buy_by", blank=True
    )
    favouriteRecipes = models.ManyToManyField(
        Recipe, related_name="favourite_to", blank=True
    )
    profilePicture = models.ImageField(
        upload_to="images/profilePicture/", blank=True, null=True
    )
...
class Review(models.Model):
    ...
    dislikedBy = models.ManyToManyField(
        User, related_name="disliked_by_reviews", blank=True
    )
...
class RecipeComponent(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.order:
            last_order = RecipeComponent.objects.filter(recipe=self.recipe).aggregate(
                models.Max("order")
            )["order__max"]
            self.order = (last_order + 1) if last_order else 1
        super().save(*args, **kwargs)
...
class RecipeIngredient(models.Model):
    ...
    ingredient = models.ForeignKey(
        Ingredient, on_delete=models.CASCADE, related_name="units"
    )
    unit = models.CharField(max_length=15, choices=UNIT_CHOICES)
    quantity = models.DecimalField(max_digits=10, decimal_places=2)
    recipeComponent = models.ForeignKey(
        RecipeComponent, related_name="ingredients", on_delete=models.CASCADE
    )

    def __str__(self):
        return (
            f"{self.quantity} {self.unit} {self.ingredient.name} in {self.recipe.name}"
        )
